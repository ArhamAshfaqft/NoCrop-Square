...
function App() {
  ...
  // License & Settings
  const [licenseKey, setLicenseKey] = useState(() => localStorage.getItem('shopsnap_licenseKey') || '')
  const [isPro, setIsPro] = useState(false) // Default to false (Free) for security
  const [filenameSuffix, setFilenameSuffix] = useState(() => localStorage.getItem('shopsnap_suffix') || '_processed')
  ...
  useEffect(() => {
    const checkLicense = async () => {
      ...
      const result = await LicenseService.verify(licenseKey)
      if (result.valid) {
        setIsPro(true)
        // Ensure localStorage is synced if it came from pre-filled
      } else {
        setIsPro(false)
        if (licenseKey.length > 5) {
          // Only warn if they actually have a key typed in
  ...
  const handleActivateLicense = async () => {
    if (!licenseKey) return toast.error("Please enter a key")
  ...
  const [presets, setPresets] = useState(() => {
    ...
    if (initial.length === 0) {
      return [
        ...
        {
          id: 1,
          name: "Shopify - Product Page (2000x2000 Recommended)",
          isDefault: true,
          settings: {
            resize: { width: "2000", height: "2000", maintainAspect: true },
        ...
        {
          id: 2,
          name: "Shopify - Variant Thumbnail (800x800)",
          isDefault: true,
          settings: {
            resize: { width: "800", height: "800", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 75, active: true },
            suffix: "_shopify_thumb"
          }
        },
        {
          id: 3,
          name: "Shopify - High-Res Zoom (2400x2400)",
          isDefault: true,
          settings: {
            resize: { width: "2400", height: "2400", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 90, active: true },
            suffix: "_shopify_zoom"
          }
        },
        ...
        {
          id: 4,
          name: "WooCommerce - Store Catalog (1200x1200)",
          isDefault: true,
          settings: {
            resize: { width: "1200", height: "1200", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 75, active: true },
            suffix: "_woo_catalog"
          }
        ...
        {
          id: 5,
          name: "WooCommerce - Product Gallery (1600x1600)",
          isDefault: true,
          settings: {
            resize: { width: "1600", height: "1600", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
        ...
        {
          id: 6,
          name: "WooCommerce - Performance Mode (800px)",
          isDefault: true,
          settings: {
            resize: { width: "800", height: "800", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 70, active: true },
            suffix: "_woo_perf"
          }
        },
        ...
        {
          id: 7,
          name: "Amazon - Main Image (1600px Zoom-Ready)",
          isDefault: true,
          settings: {
            resize: { width: "1600", height: "1600", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 100, active: true }, // Amazon compresses anyway, send high quality
            suffix: "_amazon"
          }
        },
        {
          id: 8,
          name: "Amazon - A+ Content (2560px HD)",
          isDefault: true,
          settings: {
            resize: { width: "2560", height: "2560", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 90, active: true },
            suffix: "_amazon_plus"
          }
        ...
        {
          id: 9,
          name: "Amazon - Thumbnail (500x500)",
          isDefault: true,
          settings: {
            resize: { width: "500", height: "500", maintainAspect: true },
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 80, active: true },
            suffix: "_amazon_thumb"
          }
        },
        ...
        {
          id: 10,
          name: "Etsy - Listing Photo (3000px Ultra-HD)",
          isDefault: true,
          settings: {
            resize: { width: "3000", height: "3000", maintainAspect: true }, // Longest side 3000
            watermark: { text: "", position: "bottom-right", active: false },
            convert: { format: "jpg", quality: 90, active: true },
            suffix: "_etsy"
          }
  ...
  const onDrop = async (e) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (paths.length === 0) {
        ...
        if (paths.length === 1) {
          // If just one thing dropped, could be folder or file
          toast.info(`Scanning: ${paths[0]}`)
          loadFolder(paths) // now accepts array
        } else {
          toast.info(`Scanning ${paths.length} items...`)
          loadFolder(paths)
        }
      }
    }
          ...
          <div className="card">
            ...
            <div style={{ opacity: resizeSettings.active ? 1 : 0.4, pointerEvents: resizeSettings.active ? 'auto' : 'none', transition: 'opacity 0.3s' }}>
              ...
              <div className="form-group">
                ...
                <select
                  value={resizeSettings.mode || (resizeSettings.maintainAspect ? 'inside' : 'fill')}
              ...
              {watermarkSettings.type === 'text' ? (
                ...
                <div className="form-group">
                  ...
                  >
                    ...
                    />
                    <button className="btn-secondary" onClick={() => document.getElementById('logo-upload').click()}>
                    ...
                    <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '5px' }}>
                      Logo will be auto-resized to 20% of image width.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="card">
            ...
            <div style={{ opacity: renameSettings.active ? 1 : 0.4, pointerEvents: renameSettings.active ? 'auto' : 'none', transition: 'opacity 0.3s' }}>
              <p style={{ color: 'var(--text-secondary)', marginBottom: '20px' }}>
                Rename your files for better SEO (e.g., "nike-air-jordan-01.jpg").
              </p>
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                  placeholder="e.g. summer-collection-shirt"
        ...
        return (
          <div className="card">
            ...
            <div className="section mt-4">
              <h3>License</h3>
              <div className="status mb-2">Current Plan: <span className={isPro ? 'text-pro' : ''}>{isPro ? 'Pro Plan' : 'Free Plan'}</span></div>
              <div className="form-group">
                <label>Enter License Key</label>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <input type="text" value={licenseKey} onChange={e => setLicenseKey(e.target.value)} placeholder="Pro key..." />
            ...
            <div className="section mt-4">
              ...
              <div className="form-group">
                ...
                <p className="helper-text">Appended to processed files (e.g. image_processed.jpg)</p>
            ...
            <div className="section mt-4" style={{ borderTop: '1px solid var(--border-color)', paddingTop: '20px' }}>
              ...
              <button
                className="btn-secondary"
                onClick={() => {
                  window.open('mailto:support@shopsnap.io?subject=OptiSnap Support Request')
                }}
