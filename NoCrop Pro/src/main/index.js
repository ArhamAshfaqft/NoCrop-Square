...
import icon from '../../resources/icon.png?asset' // Icon Asset
import fs from 'fs'
...
function createWindow() {
  ...
  if (!gotTheLock) {
    ...
    app.whenReady().then(() => {
        electronApp.setAppUserModelId('com.nocropsquare.app')
        ...
        if (process.argv.length > 1) {
             ...
             // In dev, the first arg is often "." or the script path. Ignore specific dev artifacts.
             const pathArg = possibleArgs.find(arg => 
                !arg.startsWith('--') && 
                arg !== '.' && 
                !arg.includes('electron.exe') &&
                fs.existsSync(arg)
             )
             
             if (pathArg) {
                 // Check if it's a real file/folder, not just CWD
                 try {
                    const stats = fs.lstatSync(pathArg)
        ...
        ipcMain.handle('system:toggleContextMenu', async (event, enable) => {
            ...
            if (enable) {
                try {
                    ...
                    await regAdd('HKCU\\Software\\Classes\\Directory\\shell\\NoCrop Square', '', 'Send to NoCrop Square')
                    await regAdd('HKCU\\Software\\Classes\\Directory\\shell\\NoCrop Square', 'Icon', appPath)
                    await regAdd('HKCU\\Software\\Classes\\Directory\\shell\\NoCrop Square\\command', '', `"${appPath}" "%1"`)

                    // 3. Single Files (*\shell) - Optional, maybe just folders? User asked for "selects images or folders"
                    // Let's add for files too
                    await regAdd('HKCU\\Software\\Classes\\*\\shell\\NoCrop Square', '', 'Send to NoCrop Square')
                    await regAdd('HKCU\\Software\\Classes\\*\\shell\\NoCrop Square', 'Icon', appPath)
                    await regAdd('HKCU\\Software\\Classes\\*\\shell\\NoCrop Square\\command', '', `"${appPath}" "%1"`)
                ...
                await regDel('HKCU\\Software\\Classes\\Directory\\shell\\CraftSnap')
                await regDel('HKCU\\Software\\Classes\\*\\shell\\CraftSnap')
                
                // Remove New Keys
                await regDel('HKCU\\Software\\Classes\\Directory\\shell\\NoCrop Square')
                await regDel('HKCU\\Software\\Classes\\*\\shell\\NoCrop Square')
                return true
            }
        })
  ...
  ipcMain.handle('image:process', async (event, { files, settings, isPro }) => {
     ...
     for (let i = 0; i < files.length; i++) {
       ...
       try {
         ...
         // Resize Logic (Advanced: Blur, Contain, Cover)
         if (settings.resize?.active) {
            ...
            const mode = settings.resize.mode || 'cover'
            const bgColor = settings.resize.background || '#ffffff'

            if (width && height) {
                if (mode === 'blur') {
                    // BLURRED BACKGROUND EFFECT
                    // 1. Create Background (Cover + Blur)
                    const backgroundBuffer = await pipeline
                        .clone()
                        .resize({ width, height, fit: 'cover' })
                        .blur(30) // Strong blur
                        .modulate({ brightness: 0.9 }) // Slight dim for contrast
                        .toBuffer();

                    // 2. Create Foreground (Inside)
                    const foregroundBuffer = await pipeline
                        .clone()
                        .resize({ width, height, fit: 'inside' })
                        .toBuffer();

                    // 3. Composite
                    pipeline = sharp(backgroundBuffer).composite([{ input: foregroundBuffer, gravity: 'center' }]);
                    ...
                    // STANDARD (Cover / Inside / Fill)
                    // Sharp supports: cover, contain, fill, inside, outside
                    pipeline = pipeline.resize({ width, height, fit: mode })
                    operationsPerformed = true;
                }
            } else if (width || height) {
                // If only one dimension, fallback to standard resize
                pipeline = pipeline.resize({ width, height, fit: mode === 'inside' ? 'inside' : 'cover' })
                operationsPerformed = true
            }
         }
         ...
         if (settings.watermark?.active) {
            ...
            if (wmType === 'image' && settings.watermark.imagePath && fs.existsSync(settings.watermark.imagePath)) {
                 ...
                 try {
                     ...
                     // MANUAL POSITIONING CALCULATION (To support Offsets)
                     const logoMeta = await sharp(finalLogoBuffer).metadata()
                     const lw = logoMeta.width
                     const lh = logoMeta.height
                     
                     let top = 0;
                     let left = 0;
                     const pos = (settings.watermark.position || 'center').toLowerCase()

                     // Base Coordinates
                     if (pos === 'center') {
                         left = Math.floor((mainWidth - lw) / 2)
                         top = Math.floor((maxHeight - lh) / 2) // Need metadata.height!
                     } else if (pos === 'top-left') {
                         left = Math.floor(mainWidth * 0.05) // 5% padding
                         top = Math.floor(maxHeight * 0.05)
                     } else if (pos === 'top-right') {
                         left = Math.floor(mainWidth * 0.95 - lw)
                         top = Math.floor(maxHeight * 0.05)
                     } else if (pos === 'bottom-right') {
                         left = Math.floor(mainWidth * 0.95 - lw)
                         top = Math.floor(maxHeight * 0.95 - lh)
                     } else if (pos === 'bottom-left') {
                         left = Math.floor(mainWidth * 0.05)
                         top = Math.floor(maxHeight * 0.95 - lh)
                     } else {
                         // Fallback Center
                         left = Math.floor((mainWidth - lw) / 2)
                         top = Math.floor((maxHeight - lh) / 2)
                     }

                     // Apply User Nudge Offsets
                     const offsetX = parseInt(settings.watermark.offsetX) || 0
                     const offsetY = parseInt(settings.watermark.offsetY) || 0
                     
                     left += offsetX
                     top += offsetY

                     // Composite using explicit coordinates
